...
`rollinggo-hotel` is an MCP (Model Context Protocol) hotel search service.
This document is updated against the latest live `rollinggo-hotel` tool behavior (verified on 2026-02-26).

In addition to the online HTTP MCP service, a `stdio` package named `rollinggo-mcp` is also provided for MCP client usage through `npx` or `uvx`.
...
If you do not want to self-host the HTTP server, you can use the `rollinggo-mcp` `stdio` package in your MCP client.
...
Run directly after the npm package is published:
...
### 3. Environment variables for the stdio package

The published `rollinggo-mcp` `stdio` package reads API credentials from environment variables:

- preferred: `ROLLINGGO_API_KEY`
- compatible fallback: `AIGOHOTEL_API_KEY`
- optional override: `ROLLINGGO_BASE_URL`
...
`checkInParam` sub-fields:

- `adultCount` (number, optional, default: `2`): Adults per room.
- `checkInDate` (string, optional, format `YYYY-MM-DD`): Check-in date. If omitted or earlier than today, it falls back to tomorrow; invalid format returns a parameter error.
- `stayNights` (number, optional, default: `1`): Number of nights.

`filterOptions` sub-fields:

- `distanceInMeter` (number, optional): POI distance in meters; default `5000` when POI distance filtering is active.
...
- `occupancyParam` (object, optional): Occupancy and room-count parameters.
- `localeParam` (object, optional): Country and currency parameters.

`dateParam` sub-fields:
